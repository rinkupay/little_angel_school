...
import FeeSummaryPopup from "../payment/FeeSummaryPopup";
...
const Payment = () => {
  ...
  const { receipt } = useSelector((state) => state.paymentReceipt);
  ...
  // HANDLE MEMO VIEW
  const handleMemoView = async(id)=> {
     // FETCH SINGLE TRANSACTION TO PREVIEW

    await dispatch(fetchSingleStudentTransaction(id))

    setShowPopup(true)

  }

  // HANDLE FEE SUMMARY POPUP
                    ...
                    <table className="student-table">
                      <thead>
                        <tr>
                          ...
                          <th className="middle-box">View</th>
                      ...
                      <tbody>
                        {fees?.map((fee, index) => (
                          <tr key={index}>
                            ...
                            <td className="third-box memo-view" onClick={()=>handleMemoView(fee._id)} ><CiMemoPad size={24}/></td>
