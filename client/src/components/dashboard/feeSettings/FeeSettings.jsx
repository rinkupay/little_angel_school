...
const FeeSettings = () => {
  ...
  // State for Medicine Fee Collection Month
    const [medicineFeeMonth, setMedicineFeeMonth] = useState([]);
  ...
  const handleLibraryFeeMonth = (e) => {
   ...
   // Handle Medicine Fee Month
  const handleMedicineFeeMonth = (e) => {
    const { value, checked } = e.target;
    setMedicineFeeMonth((prevMonth) => {
      if (checked) {
        return [...prevMonth, value]; // Add month to array if checked
      } else {
        return prevMonth.filter((month) => month !== value); // Remove month from array if unchecked
  ...
  const handleExamAdmissionUpdate = () => {
    const data = { admissionFeeMonth, examFeeMonth, libraryFeeMonth, medicineFeeMonth };
    ...
    if (fee && fee.classWise) {
      ...
      if (classData) {
        setFees((prevFees) => ({
          ...
          medicalFee: classData?.medicalFee || 0,
        }));
      }
    }
  }, [className, fee]);
  ...
  useEffect(() => {
    if (fee) {
      ...
      setMedicineFeeMonth(fee?.monthSet?.medicineFeeMonth || []);
    }
  }, [fee, className]);
      ...
      <div className="dashboard-right">
        ...
        <div className="fee-wrapper">
          ...
          <div className="fee-container-right">
            ...
            {/* Medicine Fee Collection Months */}
            <div className="right-fee-wrapper">
              <div className="fee-collect">
                <label className="fee-label" htmlFor="fee-label">
                  Medicine Fee
                  ...
                  {monthsName.map((monthName) => (
                    <li key={monthName}>
                      ...
                      <input
                        ...
                        type="checkbox"
                        value={monthName}
                        onChange={handleMedicineFeeMonth}
                        checked={medicineFeeMonth?.includes(monthName)}
                        disabled={isAvailable}
                      />
                    </li>
                  ))}
