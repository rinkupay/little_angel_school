...
const schoolDetailModel = require("../models/schoolDetailModel");
...
if (!schoolDetails) {
  return res.status(400).json({ success: false, message: "School details not found." });
}
  ...
  try {
    ...
    // Find Medicine fee
    const medicineFeeAmount = feeStructure.classWise.find(
      (amount) => amount.className === std
    )
    if(medicineFeeAmount){
      medicineFee = medicineFeeAmount.medicalFee;
    }
    ...
    // Months where exam fee is applicable
    const medicineFeeMonths = feeStructure.monthSet.medicineFeeMonth;
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const year = Number(
  i < 10
    ? schoolDetails.sessionDetails.startDate.slice(0, 4)
    : schoolDetails.sessionDetails.endDate.slice(0, 4)
);
      ...
      const finalMedicineFee = medicineFeeMonths.includes(monthName) ? medicineFee : 0;
      const totalAmount = baseTotalAmount + finalAdmissionFee + finalExamFee + finalMedicineFee;
